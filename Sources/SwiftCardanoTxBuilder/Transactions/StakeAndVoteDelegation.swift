...
public extension TxBuilder.Transactions {
    ...
    /// Creates a transaction that delegates a stake address to a pool operator and a DRep.
    /// - Parameters:
    ...
    /// - Returns: The created transaction with the stake and vote delegation certificate.
    /// - Throws: CardanoTxBuilderError if the transaction could not be created.
    func stakeAndVoteDelegation(
        ...
        let stakeVoteDelegateCertificate = StakeVoteDelegate(
        ...
        self.txBuilder.certificates = [
            .stakeVoteDelegate(stakeVoteDelegateCertificate)
        ]
