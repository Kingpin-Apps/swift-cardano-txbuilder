...
public extension TxBuilder.Transactions {
    ...
    /// Create a transaction that deregisters a stake address.
    /// - Parameters:
    ...
    func stakeAddressderegistration(
        ...
        // 2. Check if stake address is registered
        ...
        if stakeAddressInfo.isEmpty,
           ...
           !active {
            throw CardanoTxBuilderError.invalidTransaction("Stake-Address: \(stakeAddress) is not registered on the chain!")
        }
        
        // 3. Create stake credential and deregistration certificate
        let stakeCredential = StakeCredential(
            credential: .verificationKeyHash(try stakeVerificationKey.hash())
        )
        let stakeDeregistrationCertificate = StakeDeregistration(
        ...
        // 5. Add the stake deregistration certificate
