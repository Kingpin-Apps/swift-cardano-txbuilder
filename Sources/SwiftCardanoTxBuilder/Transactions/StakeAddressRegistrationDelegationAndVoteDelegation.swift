...
public extension TxBuilder.Transactions {
    ...
    /// Create a stake address registration, stake delegation, and vote delegation transaction.
    /// - Parameters:
    ///   - stakeVerificationKey: The stake verification key for the stake address to register
    ///   - poolOperator: The pool operator to delegate staking rights to
    ///   - drep: The DRep to delegate voting rights to
    ...
    /// - Returns: A `Transaction` object representing the created transaction.
    /// - Throws: CardanoTxBuilderError if the transaction could not be created.
    func stakeAddressRegistrationStakeDelegationAndVoteDelegation(
        ...
        // 3. Create stake credential and certificate
        let stakeCredential = StakeCredential(
            credential: .verificationKeyHash(try stakeVerificationKey.hash())
        )
        let stakeVoteRegisterDelegateCertificate = StakeVoteRegisterDelegate(
        ...
        self.txBuilder.certificates = [
            .stakeVoteRegisterDelegate(stakeVoteRegisterDelegateCertificate)
        ]
