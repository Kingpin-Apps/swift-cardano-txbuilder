...
public extension TxBuilder.Transactions {
    ...
    /// Create a transaction that registers a stake address and delegates voting rights to a DRep.
    /// - Parameters:
    ...
    /// - Returns: A `Transaction` object representing the stake address registration and vote delegation transaction.
    /// - Throws: CardanoTxBuilderError if the transaction could not be created.
    func stakeAddressRegistrationAndVoteDelegation(
        ...
        let voteRegisterDelegateCertificate = VoteRegisterDelegate(
        ...
        self.txBuilder.certificates = [
            .voteRegisterDelegate(voteRegisterDelegateCertificate)
        ]
